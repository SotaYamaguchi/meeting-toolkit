...
以下のルールは静的解析（golangci-lint）で自動チェックされます：

- ✅ **全ての公開関数にGoDocコメントを追加** - `revive`でチェック
- ✅ **パッケージレベルのコメントを各パッケージに記載** - `revive`でチェック
- ⚠️ **エラーメッセージは日本語（ユーザー向け）** - 自動チェック不可、レビューで確認
- ℹ️ **標準ライブラリのみ使用（外部依存なし）** - `go.mod`で保証

### 静的解析の実行

```bash
make lint  # CI/CDと同じチェックをローカルで実行
```
